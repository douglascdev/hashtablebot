...
def main(token, channels, log_level):
    ...
    as specified in the project's [setup file](https://github.com/douglascdev/hashtablebot/blob/343154016d1e32002ab417ab8f52456e967fe803/setup.py#L21).

    The parameters/environment variables are gathered here by [Click](https://click.palletsprojects.com/en/8.1.x/)
    and passed to the HashTableBot class.

    :param token: the OAuth Access Token provided by twitch(see https://dev.twitch.tv/docs/authentication/getting-tokens-oauth).
    :param channels: A comma-separated list of twitch channels the bot should join
    :param log_level: set the logging level(see https://docs.python.org/3/library/logging.html#levels).
    :return:
    """
